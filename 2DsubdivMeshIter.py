...
class tile:
    ...
    def gen(self,cap=True,connect=True):
        pts = []
        newPts = []
        faces = []
        ...
        return profile
    def subDiv(self,height,cap=True,connect=True):
        ...
        divTiles.append(tile([pts[4],pts[0],cnt,pts[1]],self.thickness,height+self.z[2],self.z[2]))
        divTiles.append(tile([pts[0],pts[5],pts[3],cnt],self.thickness,height+self.z[2],self.z[2]))
        divTiles.append(tile([cnt,pts[3],pts[6],pts[2]],self.thickness,height+self.z[2],self.z[2]))
        divTiles.append(tile([pts[1],cnt,pts[2],pts[7]],self.thickness,height+self.z[2],self.z[2]))
        for i in range(len(divTiles)):
            divTiles[i].gen(cap,connect)
        return divTiles
    def cntPt(self):
        sum = [0,0,0]
        for i in range(len(self.vert)):
